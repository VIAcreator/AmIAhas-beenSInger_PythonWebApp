...
def extract_prefix_creator(title: str) -> str | None:
    ...
    if m:
        name = m.group(1).strip()
        if name.upper() not in ("IA",) and (len(name) >= 3 or name in KNOWN_CREATORS):
            return name
    return None
...
def _load_keyword_map(path: str = None) -> pd.DataFrame:
    """加载关键词映射表，按关键词长度降序排序。"""
    global _keyword_map
    if _keyword_map is None:
        ...
        if path is None:
            ...
            path = os.path.join(base, "data", "song_keywords.csv")
...
def parse_titles(df: pd.DataFrame) -> pd.DataFrame:
    """
    对 DataFrame 逐行解析标题，新增 song_name/original_creator/vocal_singer/is_reupload 列。
    """
    results = []
    for _, row in df.iterrows():
        r = parse_title(str(row.get("title", "")), str(row.get("tags", "")))
        results.append(r)

    df = df.copy()
    df["song_name"]        = [r["song_name"] for r in results]
    df["original_creator"] = [r["original_creator"] for r in results]
    df["vocal_singer"]     = [r["vocal_singer"] for r in results]
    df["is_reupload"]      = [r["is_reupload"] for r in results]
    return df
